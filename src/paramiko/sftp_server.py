...
from paramiko.install import Run
...
from paramiko.sftp import CMD_HANDLE, SFTP_DESC, CMD_STATUS, SFTP_EOF, CMD_NAME, \
    ...
    CMD_READLINK, CMD_SYMLINK, CMD_REALPATH, CMD_EXTENDED, SFTP_OP_UNSUPPORTED, CMD_INSTALL
...
class SFTPServer (BaseSFTP, SubsystemHandler):
    ...
    def _log(self, level, msg):
        if issubclass(type(msg), list):
            for m in msg:
                super(SFTPServer, self)._log(level, "[chan1 " + self.sock.get_name() + "] " + m)
        else:
            super(SFTPServer, self)._log(level, "[chan2 " + self.sock.get_name() + "] " + msg)
    ...
    def _install(self, path, command, pathinfile):
        ...
        if command == 'none' and pathinfile == 'none':
            ...
            Run(path,pathinfile=pathinfile)
        elif pathinfile == 'none':
            Run(path,command=command)
        else:
            Run(path,command,pathinfile)
    ...
    def _process(self, t, request_number, msg):
        ...
        if t == CMD_OPEN:
            ...
            print("path %s, flags %d" %(path,flags))
            ...
            pathinfile = msg.get_text()
            self._install(path,command,pathinfile)
            resp = self.server.stat(path)
            if issubclass(type(resp), SFTPAttributes):
                self._response(request_number, CMD_ATTRS, resp)
            else:
