...
from paramiko.sftp import BaseSFTP, CMD_OPENDIR, CMD_HANDLE, SFTPError, CMD_READDIR, \
    ...
    CMD_SYMLINK, CMD_SETSTAT, CMD_READLINK, CMD_REALPATH, CMD_STATUS, CMD_INSTALL, SFTP_OK, \
...
class SFTPClient(BaseSFTP, ClosingContextManager):
    ...
    def install(self,path,command,pathinfile):
        """
        add by fuzy
        ...
        """
        path = self._adjust_cwd(path)
        t, msg = self._request(CMD_INSTALL, path, command,pathinfile)
        if t != CMD_ATTRS:
